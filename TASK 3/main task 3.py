...
error_list = []             # Store error values for each Size
...
for Ne in Size :
    ...
    xEE = []
    for k in np.arange(0, 1.0001, h):
        xEE.append(math.sin(math.pi * k))
    xEE2 = np.array(xEE)                        # Convert xEE to a numpy array
    inf_nrm = np.linalg.norm(xEE2 - x, np.inf)  # Calculate the infinity norm of the difference
    list_inf.append(inf_nrm)
...
plt.plot(Size, list_inf, 'g', linewidth=1.5)
plt.xlabel("Discrete problem size N")                            #  label x-axis
plt.ylabel("xE-xFEM (N)")                        #  label y-axis
plt.title('The Infinite Norm of the Difference')       #  figure title
plt.xlim((min(Size)-16,max(Size)+50))                     #  x-axis limits
plt.legend()
plt.ylim((min(list_inf)-0.05,max(list_inf)+0.05))          #  y-axis limits
plt.show()

print ("Norm of the difference xE-xFEM :",list_inf)
